...
- 🔄 **Phase 2** — พอร์ต UI page logic (`components/pages/*`): 8/9 หน้า
  - ✅ ภาพรวม · จัดการสต็อก · โอนเข้าร้าน · วัตถุดิบ · สูตรขนม · บันทึกขาย · เคลม · วิเคราะห์
  - ⬜ จัดการผู้ใช้ (ยัง scaffold — รอระบบ auth/login ก่อน เพราะต้องมี profiles/login_history)
- ✅ **POS adapter — FoodStory** (แทน Ocha): sync เมนู+ยอดขายเข้า Supabase ได้แล้ว (MVP)
  ดู `deploy/docs/foodstory-integration.md` · `scripts/foodstory_{fetch,sync}.mjs`
  ⬜ เหลือ: auto-login (ตอนนี้ copy cookie มือ) · ตั้งเวลา sync · ตัดสต็อก (รอ qty_stock)
- ⬜ ยืนยัน `qty_stock` ในสูตร + ผูก recipe (BOM ชั้น 2) ให้เมนู FoodStory → เปิดตัดสต็อกอัตโนมัติ
